...
public class Data {
	...
	public void setDepthsFromRadar(CaptureData data){
		depths.clear();
		...
		for(int i=0;i<size;i++){
			...
			depths.add(new Cell(data.steps.get(i).distances.get(0), 0));
		}		
	}
	...
	public void displayInitData(){
		...
		for (int i = 0; i < depths.size(); i++) {
			cell = depths.get(i);
			if(i==0){
				System.out.printf("%d",(int)cell.depth);
			}else
			System.out.printf(";%d",(int)cell.depth);
		}
		System.out.println();
	...
	public void cluster() {
		...
		for (int i=0;i<accuracy;i++){
			tmpMaxGroup=0;
			tmpKey=0;
			if(depths==null||depths.get(0)==null)return;
