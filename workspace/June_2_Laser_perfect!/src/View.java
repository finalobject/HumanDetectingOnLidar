...
public class View extends JPanel{
	...
	public void paint(Graphics g) {
		if(data.getDepths()==null||data.getDepths().get(0)==null)return;
		super.paint(g);
		...
		g2.drawString("shift", DEFAULT_SIZE+2, 180);
		...
		for(int i=0;i<25;i++){
			if(i<7){
				g2.drawOval(DEFAULT_SIZE/2-(i+1)*50, DEFAULT_SIZE/2-(i+1)*50+shift, (i+1)*100, (i+1)*100);
			}else
			{
				g2.drawArc(DEFAULT_SIZE/2-(i+1)*50, DEFAULT_SIZE/2-(i+1)*50+shift, (i+1)*100, (i+1)*100, 
		...
		for(int i=0;i<data.getDepths().size();i++){
			x=(data.getDepths().get(i).depth)*Math.cos(-angle*i-Math.PI/4*3+Math.PI)/range*(DEFAULT_SIZE/2)+DEFAULT_SIZE/2;
			y=(data.getDepths().get(i).depth)*Math.sin(-angle*i-Math.PI/4*3+Math.PI)/range*(DEFAULT_SIZE/2)+DEFAULT_SIZE/2;
		 ...
		 if(data.isFlaged()==true){
			...
			for(int i=0;i<data.getDepths().size();i++){
				...
				if(i!=0&&before!=null&&now.flag==before.flag){
					x=(now.depth)*Math.cos(-angle*i-Math.PI/4*3+Math.PI)/range*(DEFAULT_SIZE/2)+DEFAULT_SIZE/2;
					y=(now.depth)*Math.sin(-angle*i-Math.PI/4*3+Math.PI)/range*(DEFAULT_SIZE/2)+DEFAULT_SIZE/2;
					x1=(before.depth)*Math.cos(-angle*(i-1)-Math.PI/4*3+Math.PI)/range*(DEFAULT_SIZE/2)+DEFAULT_SIZE/2;
					y1=(before.depth)*Math.sin(-angle*(i-1)-Math.PI/4*3+Math.PI)/range*(DEFAULT_SIZE/2)+DEFAULT_SIZE/2;
